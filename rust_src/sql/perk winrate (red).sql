select
	...
	concat(cast(round(sum(1.0 * loadouts.ticks_alive / players.ticks_alive) / cnt, 2) * 100 * 15 / 8 as INTEGER), '%') as popularity, -- in a game of 8 players and 15 perks, we expect 8/15 of each perk per game

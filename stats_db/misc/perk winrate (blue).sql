select
	...
	round(sum(1.0 * loadouts.ticks_alive / players.ticks_alive) / cnt, 2) as avg_players_per_game,
	concat(cast(round(sum(1.0 * loadouts.ticks_alive / players.ticks_alive) / cnt, 2) * 100 * 4 / 8 as INTEGER), '%') as popularity, -- in a game of 8 players and 4 perks, we expect 8/4 of each perk per game
...
inner join perk_names as blue_names
	on loadouts.blue_perk = blue_names.id
left join (select count(*) as cnt from ladder_games)
	on true
group by loadouts.blue_perk

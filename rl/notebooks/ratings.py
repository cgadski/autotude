...
# %%
def get_games() -> tuple[
    np.ndarray[np.int16], np.ndarray[np.int16], np.ndarray[np.int16]
]:
    games = query("""
        SELECT
...
# %%
np.sum(np.bincount(teams.flatten()) > 0)
...
def vectorize_teams(teams):
    ...
    handle_idx = {
        handle_key: i for i, handle_key in enumerate(known_players["handle_key"])
    }
...
pred_df = pd.DataFrame({"pred": np.digitize(prob, bins), "outcome": outcomes})
...
rating_df = (
    pd.DataFrame(
        {"handle": known_players["handle"], "rating": model.coef_[0, :-1] / np.log(2)}
    )
